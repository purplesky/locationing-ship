
<div align="center">
    <div class="container" >

      <form class="form-signin" action='xl.php' method="post">
        <h3 class="form-signin-heading"><FONT FACE='Times New Roman' COLOR='BLUE'size='4'>HỆ THỐNG ĐỊNH VỊ TÀU BIỂN</font></h3>
        <input type="text" name="myusername" id="myusername" class="input-block-level" placeholder="Tài khoản">
        <input type="password" name="mypassword" id="mypassword" class="input-block-level" placeholder="Mật khẩu">
        <div align="right">
		<div align="left" class="text-right" >
		</div>
		<button class="btn btn" type="submit">Đăng nhập</button></div>
</form>
